...
class Upload
{
    ...
    */
    ...
    public function setMaxImageSize($maxImageWidth = UPLOAD_IMAGE_MAX_WIDTH, $maxImageHeight = UPLOAD_IMAGE_MAX_HEIGHT)
    {
    ...
    public function setDestinationFileName($destinationFileName = 'uploadedFile.file')
    {
    ...
    public function setRegisteredMimeTypes($registeredMimeTypes = [])
    {
        if (0 == count($registeredMimeTypes)) {
            $this->registeredMimeTypes = [
...
'application/octet-stream' => '.exe, .fla, .psd (etc)',
    ...
    public function setMaxFileSize($maxFileSize = UPLOAD_MAX_FILE_SIZE)
    {
    ...
    public function printFormStart($formAction = './', $formMethod = 'POST', $formName = 'uploadForm', $formTarget = '_self', $formInlineJavaScript = '')
    {
    ...
    public function printFormField($fieldName = '') //+++
    {
        if (!$fieldName) {
    ...
    public function printFormSubmit($name = 'submit', $value = 'Upload', $formInlineJavaScript = '')
    {
    ...
    */
    ...
    {
        if (!in_array($this->HTTP_POST_FILES[$this->uploadFieldName]['type'], $this->getAllowedMimeTypes(), true)) {
    ...
    public function copyFile() // TODO check for is_writeable()
    {
        ...
        return $func($this->uploadFile, $this->uploadPath . '/' . $this->destinationFileName . '.' . mb_strtolower($ext));
    }
    ...
    { //+++
        ...
        */
        ...
        if (!$this->checkMaxFileSize()) {
            ...
            $this->uploadFail = true;
        } elseif (in_array('cannotGetImageSize', $this->getUploadErrors(), true)) {
        ...
        if (!$this->uploadFail) {
            if (preg_match('image', $this->HTTP_POST_FILES[$this->uploadFieldName]['type'])) {
    ...
    public function setUploadFileNamesArrName($value)
    {
